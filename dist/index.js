...
// ═══════════════════════════════════════════════════════════════
server.tool('compare_shipping_rates', 'Compare shipping rates across multiple carriers (UPS, FedEx, USPS, DHL) for a package. Returns cheapest and fastest options. Free — no API key needed.', {
...
server.tool('get_platform_status', 'Check CerebroChain platform health and service availability. Free — no API key needed.', {}, async () => {
...
// ═══════════════════════════════════════════════════════════════
// ── Inventory ──
server.tool('search_inventory', 'Search inventory items with filters. Returns SKU, name, quantity, location, category. Requires API key.', {
    ...
    if (!client.isAuthenticated) {
        return { content: [{ type: 'text', text: 'Error: API key required. Set CEREBROCHAIN_API_KEY environment variable.' }] };
    }
    const query = {
...
server.tool('check_stock_levels', 'Get current stock levels for a specific inventory item by ID. Returns quantity on hand, reserved, available. Requires API key.', {
...
server.tool('lookup_sku', 'Look up an inventory item by its SKU code. Returns full item details including stock, location, and pricing. Requires API key.', {
...
server.tool('get_warehouse_locations', 'List warehouse storage locations with hierarchy (zones, aisles, bins). Requires API key.', {}, async () => {
    if (!client.isAuthenticated) {
        return { content: [{ type: 'text', text: 'Error: API key required.' }] };
    }
...
// ── Orders ──
server.tool('list_orders', 'List and search orders with filters. Returns order ID, status, items, total value. Requires API key.', {
...
server.tool('get_order_status', 'Get detailed status and tracking for a specific order. Requires API key.', {
...
server.tool('get_order_statistics', 'Get order statistics and KPIs — total orders, fulfillment rate, average value, trends. Requires API key.', {}, async () => {
    if (!client.isAuthenticated) {
        return { content: [{ type: 'text', text: 'Error: API key required.' }] };
    }
...
// ── Logistics ──
server.tool('get_fleet_stats', 'Get real-time fleet KPIs — active vehicles, utilization, fuel efficiency, maintenance status. Requires API key.', {}, async () => {
    if (!client.isAuthenticated) {
        return { content: [{ type: 'text', text: 'Error: API key required.' }] };
    }
...
server.tool('list_vehicles', 'List fleet vehicles with status, location, and capacity. Requires API key.', {
...
server.tool('track_shipment', 'Get real-time tracking for a shipment route — current location, progress, ETA, stops completed. Requires API key.', {
...
server.tool('list_shipments', 'Get shipment history with analytics — 90-day view of all shipments, delivery rates, carrier performance. Requires API key.', {}, async () => {
    if (!client.isAuthenticated) {
        return { content: [{ type: 'text', text: 'Error: API key required.' }] };
    }
...
// ═══════════════════════════════════════════════════════════════
server.tool('optimize_route', 'AI-powered route optimization. Calculates optimal delivery route considering traffic, capacity, time windows, and fuel costs. Premium tool. Requires API key.', {
    ...
    if (!client.isAuthenticated) {
        return { content: [{ type: 'text', text: 'Error: API key required. Premium feature.' }] };
    }
...
server.tool('forecast_demand', 'AI-powered demand forecasting. Ask natural language questions about future inventory needs, sales trends, and supply chain patterns. Premium tool. Requires API key.', {
...
server.tool('detect_bottlenecks', 'AI-powered bottleneck detection. Identifies current and predicted supply chain bottlenecks with severity and recommendations. Premium tool. Requires API key.', {
...
server.tool('get_optimization_recommendations', 'Get AI-generated optimization recommendations for warehouse operations, logistics routes, or overall supply chain efficiency. Premium tool. Requires API key.', {
...
server.tool('get_financial_metrics', 'Get real-time financial metrics — revenue, margins, cash flow, profit/loss. Premium tool. Requires API key.', {}, async () => {
    if (!client.isAuthenticated) {
        return { content: [{ type: 'text', text: 'Error: API key required. Premium feature.' }] };
    }
...
server.tool('get_kpi_dashboard', 'Get KPI performance dashboard — targets vs actuals for all key supply chain metrics. Premium tool. Requires API key.', {}, async () => {
    if (!client.isAuthenticated) {
        return { content: [{ type: 'text', text: 'Error: API key required. Premium feature.' }] };
    }
...
server.tool('natural_language_command', 'Process a natural language command through CerebroChain AI Command Center. Supports queries like "show me all delayed shipments" or "reserve 50 units of SKU-1234". Premium tool. Requires API key.', {
...
// RESOURCES
// ═══════════════════════════════════════════════════════════════
server.resource('inventory-summary', 'cerebrochain://inventory/summary', async (uri) => {
    if (!client.isAuthenticated) {
        return {
            contents: [{
                    uri: uri.href,
...
server.resource('fleet-status', 'cerebrochain://logistics/fleet-status', async (uri) => {
    if (!client.isAuthenticated) {
        return {
            contents: [{
                    uri: uri.href,
...
server.resource('kpi-snapshot', 'cerebrochain://executive/kpi-snapshot', async (uri) => {
    if (!client.isAuthenticated) {
        return {
            contents: [{
                    uri: uri.href,
